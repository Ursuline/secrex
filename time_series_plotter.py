...
class TimeSeriesPlotter(Plotter):
    def __init__(self, conf:config.Config, req:request.Request, of:obj_func, frm:frame.Frame):
        Plotter.__init__(self, conf=conf, req=req)
    ...
    def _build_moving_average(self, figure:go.Figure, ma_type:str):
        ...
        """
        assert(ma_type in ['ema', 'sma']), f'ma_type {ma_type} not ema or sma {self.__class__name, sys.get_frame()}'
        figure.add_trace(go.Scatter(x = self._data_frame.index,
                                    y = self._data_frame[f'{ma_type.upper()}_{self._period}'],
                                    mode = 'lines',
                                    name = f'{ma_type} ({self._period} days)',
    ...
    def _build_buffer(self, figure:go.Figure):
            ...
            for sign in ('+', '-'):
                ...
                if sign == '-':
                    showlegend = False
                figure.add_trace(go.Scatter(x = self._data_frame.index,
                                            y = self._data_frame[f'EMA_{self._period}_{sign}'],
                                            mode = 'lines',
                                            name = 'buffer',
                                            line = {'color': self._config[self._plot_type]['ema']['color'],
    ...
    def _extract_title_data(self):
        ...
        title = f'{info["name"]} '
        title += f'({self._req.get_ticker()} | {info["exchange"]})'
        return title
    ...
    def _build_tx(self, figure:go.Figure):
        ...
        for tx in ['Buy', 'Sell']:
            df = self._data_frame
            df = df[df[column] == tx[0]]
            if tx == 'Buy':
                symbol = self._config[self._plot_type]['markers']['buy_symbol'],
            else:
                symbol = self._config[self._plot_type]['markers']['sell_symbol'],
            figure.add_trace(go.Scatter(x = df.index,
                                        y = df['adj_close'],
                                        mode = 'markers',
                                        name = tx,
                                        marker_line_width = self._config[self._plot_type]['markers']['line_width'],
                                        marker_size = self._config[self._plot_type]['markers']['size'],
                                        marker_color = self._config[self._plot_type]['markers'][f'{tx.lower()}_color'],
                                        marker_symbol = symbol[0],
                                        ))
    ...
    def plot(self):
        ...
        super()._build_title(fig, {'x': self._config[self._plot_type]['x_axis_title'],
                                   'y': self._config[self._plot_type]['y_axis_title'] + f" ({self._req.get_company_info()['currency-symbol']})",
                                   })
        # Output
